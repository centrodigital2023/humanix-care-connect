...
import {
  Loader2, ShieldAlert, Users, Briefcase, FileCheck, Mail,
  LayoutDashboard, ScrollText, Megaphone, MessageSquare, Sparkles, Search, Inbox, Send,
} from "lucide-react";
...
type Offer = {
  ...
  amount: number; modality: string; status: string; created_at: string;
  poster_type: string; specialty_required: string | null;
};

type Pqrs = {
  id: string; subject: string; description: string; type: string;
  ai_category: string | null; ai_priority: string | null; ai_sentiment: string | null;
  ai_summary: string | null; status: string; created_at: string;
...
function MarketplacePage() {
  ...
  const [matchResults, setMatchResults] = useState<Record<string, { user_id: string; similarity: number }[]>>({});
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "pqrs_tickets" }, () => void load())
      .subscribe();
  ...
  const load = async () => {
    const [{ data: o }, { data: t }] = await Promise.all([
      supabase.from("job_offers").select("id,title,description,city,amount,modality,status,created_at,poster_type,specialty_required").order("created_at", { ascending: false }).limit(50),
      supabase.from("pqrs_tickets").select("id,subject,description,type,ai_category,ai_priority,ai_sentiment,ai_summary,status,created_at,contact_email,contact_name").order("created_at", { ascending: false }).limit(50),
    ]);
  ...
  const runMatch = async (offerId: string) => {
    ...
    try {
      const { data, error } = await supabase.rpc("match_professionals_for_offer", {
        _offer_id: offerId, _match_count: 5, _min_similarity: 0.4,
      });
  ...
  const classifyTicket = async (id: string) => {
    setClassifying(id);
    try {
      const { error } = await supabase.functions.invoke("pqrs-classifier", { body: { ticket_id: id } });
      if (error) throw error;
  ...
  const updateTicketStatus = async (id: string, status: string) => {
    await supabase.from("pqrs_tickets").update({
      status,
  ...
  const filteredOffers = offers.filter((o) =>
    !search || o.title.toLowerCase().includes(search.toLowerCase()) || o.city.toLowerCase().includes(search.toLowerCase()),
  );
        ...
        <Tabs defaultValue="offers" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="pqrs" className="gap-2">
              <Inbox className="h-3.5 w-3.5" /> PQRS ({tickets.filter((t) => t.status !== "resolved").length})
            </TabsTrigger>
          </TabsList>
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="outline" className="text-[10px]">{o.poster_type}</Badge>
                        {o.specialty_required && <Badge variant="outline" className="text-[10px]">{o.specialty_required}</Badge>}
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">
                        {o.city} · {o.modality} · ${o.amount.toLocaleString("es-CO")}
                      </p>
                      {o.description && (
                        <p className="text-xs text-foreground/80 mt-2 line-clamp-2">{o.description}</p>
                      )}
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      <Button size="sm" variant="outline" onClick={() => runMatch(o.id)} disabled={matching === o.id} className="gap-1.5">
                        {matching === o.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3 text-biosensor" />}
                        Matchmaking IA
                      </Button>
                      <ShareButtons
                  ...
                  {matchResults[o.id]?.length ? (
                    <div className="mt-3 pt-3 border-t border-border space-y-1">
                      ...
                      {matchResults[o.id].map((m) => (
                        <Link
                          key={m.user_id} to="/profesional/$proId" params={{ proId: m.user_id }}
          ...
          <TabsContent value="pqrs" className="space-y-3">
            {filteredTickets.length === 0 ? (
              <Card className="p-8 text-center text-sm text-muted-foreground">Sin tickets PQRS.</Card>
            ) : (
              filteredTickets.map((t) => (
                <Card key={t.id} className="p-5">
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {t.ai_priority && (
                          <Badge className={`text-[10px] ${
                            t.ai_priority === "urgent" ? "bg-fuchsia-neural text-fuchsia-neural-foreground"
                              : t.ai_priority === "high" ? "bg-copper text-copper-foreground"
                        ...
                        {t.ai_sentiment && (
                          <Badge variant="outline" className="text-[10px]">{t.ai_sentiment}</Badge>
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">
                        {t.contact_name || t.contact_email || "Anónimo"} · {new Date(t.created_at).toLocaleDateString("es-CO")}
                      </p>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <Button size="sm" variant="outline" onClick={() => classifyTicket(t.id)} disabled={classifying === t.id} className="gap-1.5">
                        {classifying === t.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3 text-biosensor" />}
                        Clasificar IA
                      </Button>
