...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: pro }, { data: docs }, { data: refs }, { data: ratings }] = await Promise.all([
      ...
      admin.from("professional_documents").select("doc_type,status,ai_score").eq("user_id", user_id),
    ...
    const aiResp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "Eres analista de confianza para una plataforma de salud. Calculas un Social Trust Score (0-100) sopesando: documentos verificados, referencias, calificaciones reales, consistencia del perfil y actividad reciente. Sé estricto pero justo." },
          { role: "user", content: `PERFIL:\n${JSON.stringify({
            especialidad: pro.specialty, años: pro.years_experience, rethus: pro.rethus_verified,
            verified: pro.verified, ai_preapproved: pro.ai_preapproved, total_jobs: pro.total_jobs,
            avg_rating: pro.avg_rating, bio_len: (pro.bio ?? "").length,
    ...
    await admin.from("professional_profiles").update({
      ...
      social_trust_breakdown: { ...parsed.breakdown, strengths: parsed.strengths, weaknesses: parsed.weaknesses, recommendation: parsed.recommendation },
      social_trust_updated_at: new Date().toISOString(),
    }).eq("user_id", user_id);

    await admin.from("ai_credits_ledger").insert({ user_id: auth.userId, feature: "social-trust-score", credits_used: 2 });
