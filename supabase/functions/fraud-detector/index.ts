...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: pro }, { data: docs }, { data: profile }] = await Promise.all([
      ...
      admin.from("professional_documents").select("doc_type,file_name,status,created_at").eq("user_id", target),
      admin.from("profiles").select("full_name,email,phone,city").eq("user_id", target).maybeSingle(),
    ]);
    if (!pro) {
    ...
    if (pro.rethus_number && !/^\d{4,}$/.test(pro.rethus_number)) {
      heuristics.push({ reason: "Formato RETHUS sospechoso", severity: "medium", evidence: `Valor: ${pro.rethus_number}` });
    }
    if ((pro.years_experience ?? 0) > 50) {
      heuristics.push({ reason: "Años de experiencia improbables", severity: "high", evidence: `${pro.years_experience} años` });
    }
    const hasRethusDoc = (docs ?? []).some(d => d.doc_type === "rethus");
    if (pro.rethus_number && !hasRethusDoc) {
      heuristics.push({ reason: "Declara RETHUS pero no subió documento", severity: "medium", evidence: "Falta doc rethus" });
    }
    if (pro.specialty && (pro.bio ?? "").length < 20) {
      heuristics.push({ reason: "Bio extremadamente corta", severity: "low", evidence: `${(pro.bio??"").length} chars` });
    }
    ...
    const aiResp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content:
            "PERFIL:\n" + JSON.stringify({
              nombre: profile?.full_name, ciudad: profile?.city,
              especialidad: pro.specialty, sub: pro.sub_specialties,
              años: pro.years_experience, rethus: pro.rethus_number,
              bio: pro.bio, certificaciones: pro.certifications,
    ...
    if (allFlags.length) {
      await admin.from("fraud_flags").insert(allFlags.map((f) => ({
        user_id: target,
        reason: f.reason,
        severity: f.severity,
    ...
    await admin.from("ai_credits_ledger").insert({
      user_id: auth.userId, feature: "fraud-detector", credits_used: 2,
    });
